...
const FoodDetails = () => {
  ...
  const handleSubmitRequest = (e, _id) => {
    ...
    axiosInstance.post("/food-request", newRequestFood).then((result) => {
      const data = result.data;
      if (data.insertedId) toast.success("Food Request Successfull");
      e.target.reset();
  ...
  const handleAccept = (_id, foodId) => {
    const accepted = { status: "Accepted" };
    const donated = { food_status: "Donated" };
    axiosInstance
      .patch(`/food-request/${_id}`, accepted)
      .then((result) => {
        const data = result.data;
        ...
        if (data.modifiedCount) {
          axiosInstance
            ...
            .then((result) => {
              ...
              console.log("doneted", result.data);
      ...
      <div className="w-full md:w-6/12 mx-auto my-20">
        <div className="flex flex-col gap-10  ">
            ...
            <div className="flex items-center gap-10">
              ...
              <div className="">
                <p className="text-xl mb-3 font-semibold">
                  Name: <span className="font-normal">{donator_name}</span>
                </p>
                <p className="text-xl font-semibold">
                  Email: <span className="font-normal">{donator_email}</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="border shadow-xl border-secondary/10 rounded-2xl p-5">
            ...
            <div className="mt-4">
              ...
              <div className="mt-4">
                <p className="text-xl font-semibold">
                  Food Name: <span>{food_name}</span>
                </p>
                <p className="text-xl font-semibold">
                  Food Status:{" "}
                  <span className="text-green-500 badge">{food_status}</span>
                </p>
                <p className="text-xl font-semibold">
                  Food Quantity: Serves{" "}
                  <span className="">{food_quantity}</span> people
                </p>
                <p className="text-xl font-semibold">
                  pickup Location: <span>{pickup_location}</span>
                </p>
                <p className="text-xl font-semibold">
                  Expire Date: <span>{expire_date}</span>
                </p>
        ...
        <dialog ref={modalRef} className="modal">
          <div className="modal-box">
            ...
            <h1 className="text-3xl text-center  mb-10 mt-3 text-secondary font-bold">
              Update Food
            </h1>
            <form
